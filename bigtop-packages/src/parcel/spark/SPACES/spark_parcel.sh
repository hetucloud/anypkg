...
usr_lib_spark=${INSTALL_PATH}/${PKG_NAME}
var_lib_spark=${INSTALL_PATH}/${PKG_NAME}
etc_spark=${INSTALL_PATH}/etc/${PKG_NAME}

bin_dir="${usr_lib_spark}/bin"
man_dir="${usr_lib_spark}/man"
doc_dir="${usr_lib_spark}/doc"

doc_spark=${doc_dir}/spark-${PKG_VERSION}

# No prefix directory
np_var_log_spark=${INSTALL_PATH}/var/log/${PKG_NAME}
np_var_run_spark=${INSTALL_PATH}/var/run/${PKG_NAME}
np_etc_spark=${INSTALL_PATH}/etc/${PKG_NAME}
...
tar -czvf $PARCELS_DIR/spark_$PARCEL_VERSION.parcel  -C $PARCEL_INSTALL_PREFX .

# Docker build image
# Note: That docker needs to be run as a root user
pushd "$PARCEL_INSTALL_PREFX"
if [ -n "$DOCKER_CREDENTIALS" ]; then
  docker build -t hetudb/spark:$PARCEL_VERSION -f $PARCEL_SPACES_DIR/Dockerfile .
  echo ${DOCKER_CREDENTIALS} | docker login -u ${DOCKER_USER} --password-stdin
  docker push hetudb/spark:$PARCEL_VERSION
fi
popd
